...
from tinygrad.helpers import DType, dtypes, prod, DEBUG
...
def load_direct(
  mdl:Tensor, state_dict:Dict[str, Any], strict=False, # from model
  ...
  for k, v in state_dict.items():
    obj = get_child(mdl, k, strict)
    if not obj: continue
    dat = v.detach().numpy()

    if skip_load_fxn(k, obj, dat): continue
    assign_fxn(obj, dat)

def quantize_std_scalar(
  mdl:Tensor, state_dict:Dict[str, Any], # from model
  bits:int, group_size:int=64, store_dtype:DType=dtypes.uint16, sigma:float=3.0, cache_dirpath:Optional[str]=None, strict=False, # from caller through kwargs
  skip_load_fxn:SkipLoadFxn_t=lambda k, o, d: False, skip_quant_fxn:SkipQuantFxn_t=lambda k, o, d: False, assign_fxn:AssignFxn_t=lambda o, d: o.assign(d), # optional overwrites for model
):
  ...
  for k, v in tqdm(state_dict.items()) if DEBUG >= 1 else state_dict.items():
    obj = get_child(mdl, k, strict)
    if not obj: continue
    dat = v.detach().numpy()

    if skip_load_fxn(k, obj, dat):  continue
    if skip_quant_fxn(k, obj, dat): assign_fxn(obj, dat)
    else:
      qinfo = []
      cache_filepath = None
      if cache_dirpath is not None:
      ...
      if len(qinfo) == 0:
        ...
        bias  = (averages - (std_devs * sigma)).reshape(-1, 1).astype(np.float16)
        scale = (std_devs * (2 * sigma / (N + 1))).reshape(-1, 1).astype(np.float16)

        zeros = scale == 0.0
        scale[zeros] = 0.0001
        bias[zeros] -= 0.0001*(N/2)

        index_dat = np.clip(np.round((grouped_dat - bias) / scale).astype(np.int64), 0, N-1).astype(store_dtype.np)
