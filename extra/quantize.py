...
def _kmeans_slice(args):
  X, N, bits, loop, out_dtype, store_dtype = args
  kmeans = KMeans(N, n_init='auto')
  kmeans.fit(X)
  cluster = np.array(kmeans.cluster_centers_, dtype=out_dtype).reshape(-1)
...
def quantize_k_means_sliced(
  mdl:Tensor, state_dict:Dict[str, Any], # from model
  bits:int, max_chunk_size=6e60, cache_dirpath:Optional[str]=None, check_values:bool=False, check_composition:bool=False, # from caller through kwargs
  skip_load_fxn:SkipLoadFxn_t=lambda k, o, d: False, skip_quant_fxn:SkipQuantFxn_t=lambda k, o, d: False, assign_fxn:AssignFxn_t=lambda o, d: o.assign(d), # optional overwrites for model
):
  assert bits >= 2 and bits <= 8, f"bits must be between 2 and 8, got {bits}"
  if cache_dirpath: cache_dirpath += f"/q{bits}"
  ...
  for k, v in tqdm(state_dict.items()):
    ...
    if skip_quant_fxn(k, obj, dat) or (QUANTIZE_COUNT >= 0 and count >= QUANTIZE_COUNT):
      ...
      if len(qinfo) == 0:
        ...
        flat = np.array(dat.flatten(), out_dtype)

        chunk_count = 1
        chunk_size  = flat.shape[0]
        while chunk_size > max_chunk_size and chunk_count < 63:
          chunk_count *= 2
          chunk_size = chunk_size // 2
          assert chunk_size * chunk_count == flat.shape[0]
          assert chunk_size % loop == 0
        
        # if DEBUG >= 1: print(chunk_count, chunk_size)
        r = trange(chunk_count) if chunk_count > 63 and DEBUG >= 1 and False else range(chunk_count)
        ...
        # args = []
        # for i in r:
        #   X = np.zeros((chunk_size, 1,), out_dtype)
        #   X[:,0] = flat[i*chunk_size:(i+1)*chunk_size]
        #   args.append((X, N, bits, loop, out_dtype, store_dtype,))

        # with Pool(16) as p:
        #   qinfo = p.map(_kmeans_slice, args)
      ...
      for qdat, cluster in qinfo:
        ...
        qt = Tensor(qdat, dtype=store_dtype).apply_quant_map(cluster_t)
        tensors.append(qt)
      
      tensor = tensors[0].cat(*tensors[1:]) if len(tensors) > 1 else tensors[0]
      tensor = tensor.reshape(dat.shape).fill_temp()
